...
- RTSP 视频窗口的控制按钮和边框仅在悬停或获得焦点时显示，减少对画面的遮挡。
- 内置无人机模型改为默认隐藏；选择 Odom 后可在位姿信息框中按需启用。
- 无人机模型开关随 `.rvizweb` 保存和恢复，且只跟随位置信息框绑定的 Odom。
- 关闭内置模型不影响通过 Marker 或 MarkerArray 展示的自定义模型。

## v1.1.3 更新

- 修复 RTSP 已连接但画面不持续刷新的问题，浏览器现在会持续解析后端 MJPEG 视频帧。
- 修复视频缩放控件与画面分离、放大后难以缩小的问题，并重新布置悬浮操作按钮。
- 增加 RTSP 短暂断流自动恢复；重连期间保留最后一帧，连续失败后才提示连接中断。
- 修复 RTSP 会话长时间播放后过期，以及关闭视频、会话过期或服务退出时 FFmpeg 进程未完全清理的问题。
