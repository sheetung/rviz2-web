...
`FRONTEND_HOST`、必要时设置 `BACKEND_HOST`。RFC1918、IPv6 ULA 和链路本地
客户端默认免认证，无需配置令牌；设置
`ALLOW_UNAUTHENTICATED_LAN=false` 可显式关闭该行为。若需公网访问，必须配置
至少 32 字符的 `API_ACCESS_TOKEN`。`CORS_ORIGINS` 必须精确列出实际前端 Origin。
...
CDN。`/docs` 与 `/openapi.json` 使用统一访问边界：可信局域网直接访问，公网
需要有效访问会话；`/redoc` 默认关闭。
